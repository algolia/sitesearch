...
- `agentStudio`: Use [Agent Studio](https://www.algolia.com/products/ai/agent-studio) endpoints for chat (optional, defaults to false)
...
To use [Agent Studio](https://www.algolia.com/products/ai/agent-studio) instead of the default Ask AI endpoints, set `agentStudio: true`. Chat will then use the Agent Studio completions API with your API key and application ID.
...
- **Keyboard shortcut**: Press `Cmd+K` (Mac) / `Ctrl+K` (Windows) to open the search modal
- **Keyboard navigation**: Use arrow keys to navigate results, `Enter` to select, `Escape` to close
- **AI chat**: Switch to chat mode to ask follow-up questions powered by Algolia's Ask AI
- **Accessible**: Full keyboard support and ARIA labels for screen readers
...
<TypeTable
  type={{
    ...
    agentStudio: {
      description: "When true, route Ask AI requests through Agent Studio endpoints (https://{applicationId}.algolia.net/agent-studio). Uses API key + application ID; no chat token. When false or unset, uses default Ask AI endpoints (token-based).",
      type: "boolean",
      default: false,
      required: false,
    },
